<?php 
session_start();// connect to database$db = mysqli_connect​(​'localhost'​, 'root', '', 'multi_login');// variable declaration​$username = "";$email    = "";$errors   = array(); 
// call the register() function if register_btn is clickedif (​isset​(​$_POST​[​'register_btn'​]​)​) {
	...
	if ($password_1 != $password_2) {
		...
		if (​isset​(​$_POST​[​'user_type'​]​)​) {
			...
			// get id of the created user			$logged_in_user_id = mysqli_insert_id​(​$db​)​;

			$_SESSION['user'] = getUserById​(​$logged_in_user_id​)​; // put logged in user in session			$_SESSION​[​'success'​]  = "You are now logged in";
			header('location: index.php');				
		}
	}}// return user array from their idfunction getUserById($id){
	global $db;
	$query = "SELECT * FROM users WHERE id=" . $id;
	$result = mysqli_query($db, $query);

	$user = mysqli_fetch_assoc​(​$result​)​;
	return $user;}// escape stringfunction e($val){
	global $db;
	return mysqli_real_escape_string​(​$db​, trim($val));}function display_error() {
	global $errors;
...
// call the login() function if register_btn is clickedif (​isset​(​$_POST​[​'login_btn'​]​)​) {
	...
	$password = e​(​$_POST​[​'password'​]​)​;	// make sure form is filled properly	if (empty($username)) {
		array_push($errors, "Username is required");
	}
	if (empty($password)) {
		array_push($errors, "Password is required");
	}	// attempt login if no errors on form	if (count($errors) == 0) {
		$password = md5($password);

		$query = "SELECT * FROM users WHERE username='$username' AND password='$password' LIMIT 1";
		$results = mysqli_query($db, $query);

		if (​mysqli_num_rows​(​$results​) == 1) { // user found			// check if user is admin or user			$logged_in_user = mysqli_fetch_assoc​(​$results​)​;
